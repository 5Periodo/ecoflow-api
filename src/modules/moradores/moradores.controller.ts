...
import {
  CreateMoradorResponseDto,
  MoradorResponseDto,
  PerfilMoradorResponseDto,
} from './dto/morador-response.dto';
...
export class MoradoresController {
  ...
  @ApiOperation({
    ...
    description:
      'Cria um novo perfil de morador vinculado a um apartamento. Acesso exclusivo para síndicos.',
  })
  @ApiResponse({
  ...
  @ApiResponse({
    ...
    description: 'Acesso restrito ao síndico',
  })
  create(@CurrentUser() user: any, @Body() dto: CreateMoradorDto) {
    if (user.type !== 'admin')
      throw new ForbiddenException('Acesso restrito ao síndico');
  ...
  @ApiOperation({
    ...
    description:
      'Retorna a lista de todos os moradores cadastrados no condomínio do síndico logado.',
  })
  @ApiResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Retorna os dados do morador logado, incluindo pontuação, nível e histórico recente de descartes.',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  @ApiResponse({
    status: HttpStatus.FORBIDDEN,
    description: 'Rota exclusiva para moradores',
  })
  @ApiResponse({
  ...
  meuPerfil(@CurrentUser() user: any) {
    if (user.type !== 'morador')
      throw new ForbiddenException('Rota exclusiva para moradores');
  ...
  @ApiOperation({
    ...
    description:
      'Retorna os detalhes de um morador pelo ID. O morador deve pertencer ao condomínio do síndico.',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: 'Morador não encontrado neste condomínio',
  })
  @ApiResponse({
    status: HttpStatus.FORBIDDEN,
    description: 'Acesso restrito ao síndico',
  ...
  @Patch('meu-perfil')
  @ApiOperation({
    summary: 'Atualiza o perfil do morador logado (App Mobile)',
    description: 'Permite que o morador atualize seu nome e/ou senha.',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  updateMeuPerfil(@CurrentUser() user: any, @Body() dto: UpdateMoradorDto) {
    if (user.type !== 'morador')
      throw new ForbiddenException('Rota exclusiva para moradores');
    return this.moradoresService.updateMeuPerfil(user.id, dto);
  }
}
